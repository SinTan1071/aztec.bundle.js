...
const { bn128, proof: { joinSplitProof } } = require('../../../dist/aztec.bundle.js');
...
describe('AZTEC extractor tests', () => {
    ...
    beforeEach(() => {
        nIn = 5;
        nOut = 5;
        blindingScalars = joinSplitProof.generateBlindingScalars(nIn + nOut, nIn);
        ...
        generateBlindingScalars = sinon.stub(joinSplitProof, 'generateBlindingScalars').callsFake(() => blindingScalars);

        // It's a random oracle! ...sort of, if you squint a bit.
        computeChallenge = sinon.stub(joinSplitProof, 'computeChallenge').callsFake(() => {
    ...
    it('extractor can extract witnesses from two satisfying proofs over the same input string in the random oracle model', () => {
        ...
        const { commitments, m } = joinSplitProof.helpers.generateFakeCommitmentSet({ kIn, kOut });
        ...
        } = joinSplitProof.constructJoinSplit(commitments, m, sender, kPublic);
        ...
        const witnesses = joinSplitProof.extractor.extractWitness(
