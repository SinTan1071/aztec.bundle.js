...
describe('AZTEC proof construction tests', () => {
    it('proof.constructJoinSplit creates a proof with well-formed outputs', () => {
        ...
        const { proofData, challenge } = joinSplitProof.constructJoinSplit(commitments, m, randomAddress(), kPublic);
    ...
    it('proof.constructJoinSplit will throw if kPublic is malformed', () => {
        ...
        try {
            joinSplitProof.constructJoinSplit(commitments, m, randomAddress(), kPublic);
        } catch (err) {
    ...
    it('proof.constructJoinSplit will throw if m is malformed', () => {
        ...
        const { commitments } = joinSplitProof.helpers.generateFakeCommitmentSet({ kIn, kOut });

        try {
            joinSplitProof.constructJoinSplit(commitments, 500, randomAddress(), kPublic);
        } catch (err) {
