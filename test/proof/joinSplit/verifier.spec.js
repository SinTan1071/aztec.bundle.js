...
const { bn128, joinSplitProof } = require('../../../dist/aztec.bundle.js');
...
describe('AZTEC verifier tests', function describeVerifier() {
    describe('success states', function success() {
        ...
        it('proof.constructJoinSplit creates a valid join-split proof', () => {
            const kIn = [80, 60];
            const kOut = [50, 100];
            const { commitments, m, trapdoor } = joinSplitProof.helpers.generateFakeCommitmentSet({ kIn, kOut });
            const sender = randomAddress();
            const { proofData, challenge } = joinSplitProof.constructJoinSplit(commitments, m, sender, -10);
            const result = joinSplitProof.verifier.verifyProof(proofData, m, challenge, sender);
        ...
        it('validates join-split proof with 0 input notes', () => {
            ...
            const sender = randomAddress();
            const { proofData, challenge } = joinSplitProof.constructJoinSplit(commitments, m, sender, kPublic);
        ...
        it('validates join-split proof with 0 output notes', () => {
            ...
            const { commitments, m } = joinSplitProof.helpers.generateCommitmentSet({ kIn, kOut });
            const kPublic = getKPublic(kIn, kOut);
            const sender = randomAddress();
            const { proofData, challenge } = joinSplitProof.constructJoinSplit(commitments, m, sender, kPublic);
        ...
        it('validates join-split proof with uneven numberse of notes', () => {
            ...
            const { commitments, m, trapdoor } = joinSplitProof.helpers.generateFakeCommitmentSet({ kIn, kOut });
            const sender = randomAddress();
            const { proofData, challenge } = joinSplitProof.constructJoinSplit(commitments, m, sender, 0);
            const result = joinSplitProof.verifier.verifyProof(proofData, m, challenge, sender);
            expect(result.pairingGammas.mul(trapdoor).eq(result.pairingSigmas.neg())).to.equal(true);
            expect(result.valid).to.equal(true);
        });
    ...
    describe('failure states', function failure() {
        ...
        beforeEach(() => {
            ...
            parseInputs = sinon.stub(joinSplitProof, 'parseInputs').callsFake(() => { });
        });
        ...
        it('will REJECT if points not on curve', () => {
            ...
            const { valid, errors } = joinSplitProof.verifier.verifyProof(proofData, 1, challenge, sender);
            expect(valid).to.equal(false);
            expect(errors.length).to.equal(4);
            expect(errors[0]).to.equal(joinSplitProof.verifier.ERRORS.SCALAR_ZERO);
            expect(errors[1]).to.equal(joinSplitProof.verifier.ERRORS.NOT_ON_CURVE);
            expect(errors[2]).to.equal(joinSplitProof.verifier.ERRORS.NOT_ON_CURVE);
            expect(errors[3]).to.equal(joinSplitProof.verifier.ERRORS.BAD_BLINDING_FACTOR);
        });
        ...
        it('will REJECT if malformed challenge', () => {
            ...
            const { commitments, m } = joinSplitProof.helpers.generateCommitmentSet({
                kIn, kOut,
            });
            const kPublic = getKPublic(kIn, kOut);
            const sender = randomAddress();
            const { proofData } = joinSplitProof.constructJoinSplit(commitments, m, sender, kPublic);

            const result = joinSplitProof.verifier.verifyProof(
                proofData, m, `0x${crypto.randomBytes(31).toString('hex')}`, sender
            );
            expect(result.valid).to.equal(false);
            expect(result.errors.length).to.equal(1);
            expect(result.errors[0]).to.equal(joinSplitProof.verifier.ERRORS.CHALLENGE_RESPONSE_FAIL);
        });
        ...
        it('will REJECT for random proof data', () => {
            ...
            const sender = randomAddress();
            const result = joinSplitProof.verifier.verifyProof(
                proofData, 1, `0x${crypto.randomBytes(31).toString('hex')}`, sender
            );
            expect(result.valid).to.equal(false);
            expect(result.errors).to.contain(joinSplitProof.verifier.ERRORS.CHALLENGE_RESPONSE_FAIL);
        });
        ...
        it('will REJECT if kPublic > group modulus', () => {
            ...
            expect(result.valid).to.equal(false);
            expect(result.errors.length).to.equal(1);
            expect(result.errors[0]).to.equal(joinSplitProof.verifier.ERRORS.SCALAR_TOO_BIG);
        });
        ...
        it('will REJECT if note value response is 0', () => {
            ...
            expect(result.errors[0]).to.equal(joinSplitProof.verifier.ERRORS.SCALAR_ZERO);
            expect(result.errors[1]).to.equal(joinSplitProof.verifier.ERRORS.CHALLENGE_RESPONSE_FAIL);
        });
        ...
        it('will REJECT if blinding factor is at infinity', () => {
            ...
            const { commitments, m } = joinSplitProof.helpers.generateCommitmentSet({ kIn, kOut });
            const sender = randomAddress();
            const { proofData } = joinSplitProof.constructJoinSplit(commitments, m, sender, 0);
            ...
            expect(result.valid).to.equal(false);
            expect(result.errors.length).to.equal(2);
            expect(result.errors[0]).to.equal(joinSplitProof.verifier.ERRORS.BAD_BLINDING_FACTOR);
            expect(result.errors[1]).to.equal(joinSplitProof.verifier.ERRORS.CHALLENGE_RESPONSE_FAIL);
        });
        ...
        it('will REJECT if blinding factor computed from invalid point', () => {
            ...
            expect(result.errors[0]).to.equal(joinSplitProof.verifier.ERRORS.SCALAR_ZERO);
            expect(result.errors[1]).to.equal(joinSplitProof.verifier.ERRORS.SCALAR_ZERO);
            expect(result.errors[2]).to.equal(joinSplitProof.verifier.ERRORS.SCALAR_ZERO);
            expect(result.errors[3]).to.equal(joinSplitProof.verifier.ERRORS.NOT_ON_CURVE);
            expect(result.errors[4]).to.equal(joinSplitProof.verifier.ERRORS.NOT_ON_CURVE);
            expect(result.errors[5]).to.equal(joinSplitProof.verifier.ERRORS.SCALAR_ZERO);
            expect(result.errors[6]).to.equal(joinSplitProof.verifier.ERRORS.BAD_BLINDING_FACTOR);
            expect(result.errors[7]).to.equal(joinSplitProof.verifier.ERRORS.CHALLENGE_RESPONSE_FAIL);
        });
    });
});
